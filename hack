...
library:Init("tsg.lua v1.04e | .gg/WYvnhbkwAA | engo#0320")
...
local funcs = {}; do 
    ...
    function funcs.remoteCheck(tab) 
        if typeof(tab) == "table" then
            ...
            if typeof(method) == "function" and table.find(debug.getconstants(method), 1.1) then
                return
            end
    ...
    function funcs.getKeyedRemotes() 
        ...
        local ScriptNames = {
            ["1.3.4"] = "exitButtonComponent",
            ["1.3.5"] = "ClientData",
        }

        for i, v in next, getgc(true) do 
            local method = funcs.remoteCheck(v)
            if method then 
                local remote = debug.getupvalue(method, 1)
                if remote and typeof(remote) == "Instance" and remote.ClassName:match("Remote") then 
                    remotes[tostring(remote)] = v 
                end
            end
            ...
            if type(v) == "function" then
                ...
                if name == "f" then
                    hookfunc(v, function() end)
                end

                local scr = getfenv(v).script
                if scr and scr.Name == ScriptNames[Version] and scr.Parent == nil then 
                    if not table.find(debug.getconstants(v), "warn") then 
                        hookfunc(v, function() end)
                    end
                end
            end
    ...
    function funcs.teleportTo(cframe) 
        ...
        local seat = funcs.getRandomUnoccupiedBoatSeat()
        if not seat then 
            return
        end
...
do -- KILLAURA
    ...
    local players
    local highlight
    local highlightInstance = Instance.new("Highlight", game:GetService("CoreGui"))
